...
void filter_sse2(const VSFrameRef * src, VSFrameRef * dst, const LGhostData * const VS_RESTRICT d, const VSAPI * vsapi) noexcept {
    ...
    auto store = [&](const vec_t srcp, const vec_t buffer, pixel_t * dstp) noexcept {
        if constexpr (std::is_same_v<pixel_t, uint8_t>) {
            ...
            const auto result = compress_saturated_s2u(srcp + (buffer >> 7), zero_si128());
            min(result, d->peak).storel(dstp);
        } else {
...
template void filter_sse2<uint8_t>(const VSFrameRef * src, VSFrameRef * dst, const LGhostData * const VS_RESTRICT d, const VSAPI * vsapi) noexcept;
template void filter_sse2<uint16_t>(const VSFrameRef * src, VSFrameRef * dst, const LGhostData * const VS_RESTRICT d, const VSAPI * vsapi) noexcept;
template void filter_sse2<float>(const VSFrameRef * src, VSFrameRef * dst, const LGhostData * const VS_RESTRICT d, const VSAPI * vsapi) noexcept;
#endif
